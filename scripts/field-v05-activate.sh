...
if [ "$current" != "$FIELD" ]; then
    echo "FIELD v0.5 installed. Select swirldesk-field with SUPER+SHIFT+T."
    exit 0
fi

mkdir -p "$HOME/.config/gtk-3.0" "$HOME/.config/gtk-4.0" "$HOME/.config/kitty" "$HOME/.local/share/icons"
...
# Refresh toolkit preferences for apps that query GSettings directly.
...
echo "SwirlDesk FIELD v0.5 active."
echo "Restart Thunar once to load the FIELD icon overlay and GTK refinements."
